...
\begin{center}
\Large
Attention-Based Hebbian Learning Rule Provides Biological Basis for Backpropagation in the Human Brain
\end{center}
...
\end{equation}
This is our local learning rule, and it only uses information directly available to each individual neuron. While we derived our rule from the backpropagation equations, it can also be derived using local optimizations on the neuronal level. In particular, if each neuron attempts to increase the weight of its outgoing connections by changing the weights of its incoming connections, we end up with an equivalent learning rule, the derivation of which can be found below.
